...
import { DeveloperLogViewer } from "@/components/DeveloperLogViewer";
    ...
    <div className="mx-auto w-full max-w-3xl p-6 space-y-6">
          ...
          <div className="grid gap-2 max-w-xs">
            <div className="flex items-center gap-3">
              <input
                ...
                onChange={(e) => setSettings((s) => ({ ...s, openInBrowser: e.target.checked }))}
                className="h-4 w-4 rounded border-input"
              />
              <Label htmlFor="openInBrowser" className="cursor-pointer">
          ...
          <div className="grid gap-2 max-w-xs">
            <div className="flex items-center gap-3">
              <input
                type="checkbox"
                id="smartTagging"
                checked={settings.experimentalFeatures?.smartTagging === true}
                onChange={(e) => setSettings((s) => ({ 
                  ...s, 
                  experimentalFeatures: { 
                    ...s.experimentalFeatures, 
                    smartTagging: e.target.checked 
                  } 
                }))}
                className="h-4 w-4 rounded border-input"
              />
              <Label htmlFor="smartTagging" className="cursor-pointer">
                Smart Tagging
              </Label>
            </div>
            <p className="text-xs text-muted-foreground">
              Automatically convert the first word of a task to a project/tag when you press spacebar. 
              Only happens once per task creation.
            </p>
          </div>
          ...
          <div className="grid gap-2 max-w-xs">
            <div className="flex items-center gap-3">
              <input
                type="checkbox"
                id="dateShortcuts"
                checked={settings.experimentalFeatures?.dateShortcuts === true}
                onChange={(e) => setSettings((s) => ({ 
                  ...s, 
                  experimentalFeatures: { 
                    ...s.experimentalFeatures, 
                    dateShortcuts: e.target.checked 
                  } 
                }))}
                className="h-4 w-4 rounded border-input"
              />
              <Label htmlFor="dateShortcuts" className="cursor-pointer">
                Date Picker Shortcuts
              </Label>
            </div>
            <p className="text-xs text-muted-foreground">
              Show "Today", "Tomorrow", and "This Week" buttons next to the date picker for quick date selection.
            </p>
          </div>

          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <p className="text-sm text-yellow-800">
              <strong>⚠️ Experimental Features:</strong> These features are in beta testing. 
              They may change or be removed in future updates. Use at your own discretion.
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            🔧 Developer Tools
            <span className="text-xs bg-red-100 text-red-800 px-2 py-1 rounded-full">DEV</span>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-2">
            <div className="flex items-center justify-between">
              <div>
                <Label className="text-sm font-medium">Developer Logs</Label>
                <p className="text-xs text-muted-foreground">
                  View complete application logs including console output, errors, and debug information.
                </p>
              </div>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setShowDeveloperLogs(true)}
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <p className="text-sm text-red-800">
              <strong>⚠️ Developer Tools:</strong> These tools are for debugging and development purposes. 
              They may contain sensitive information and should only be used by developers.
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <DeveloperLogViewer 
        open={showDeveloperLogs} 
        onOpenChange={setShowDeveloperLogs} 
      />
    </div>
  );
}