...
async function loadTasks(): Promise<Task[]> {
  try {
    ...
    if (tauri) {
      ...
      // Web version with localStorage fallback
      ...
      if (!res.ok) {
        ...
        if (typeof window !== "undefined") {
          ...
          if (backup) {
            console.log("📦 Loading tasks from localStorage backup (counters)");
    ...
    if (typeof window !== "undefined") {
      try {
        const backup = localStorage.getItem("tasks_backup");
        if (backup) {
          console.log("📦 Loading tasks from localStorage backup (counters - error)");
          const data = JSON.parse(backup);
          return Array.isArray(data) ? data as Task[] : [];
        }
      } catch (localError) {
